#!/bin/bash
# E07: E06's learner (GRU, standard defaults) INITIALISED from E06 update 3140 (params only:
# fresh optimizer, fresh lr schedule, own 3000-update budget),
...
SEED=${SEED:-0}
INIT=${INIT:-/mnt/nfs/projects/ahriuwu-lanerl-jax/lanerl_jax/runs/E06_mirror_wave_gru_ent3/seed0/server-farm-s0-20260926-151710-1a89b8e4/eval_u3140.msgpack}
...
ARGS=(--envs 6 --opponent frozen --opponent-ckpt "$OPP" --start-near-wave --rollout 256 --updates 3000
      --step-ticks 6 --episode-s 600 --preset standard --core gru --lr-anneal
      --xp-weight 0.002 --no-snap-clicks --minibatches 3 --ckpt-every 20
      --save-updates 1000 2000 --seed "$SEED" --port-base $((23300 + 40 * SEED))
      --out lanerl_jax/runs/E07_frozen_opponent/seed$SEED)
if [ -n "$INIT" ]; then [ -f "${INIT/\/mnt\/nfs/\/srv\/nfs}" ] || { echo "INIT must exist: $INIT" >&2; exit 2; }; ARGS+=(--init-from "$INIT"); fi
