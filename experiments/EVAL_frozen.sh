...
set -e
: "${CKPT:?set CKPT}"; ENVS=${ENVS:-5}; EPISODES=${EPISODES:-1}; OPP=${OPP:-mirror}; WAVE=${WAVE:-1}; SEED=${SEED:-0}
...
exec srun -p cpu -w desktop --cpus-per-task=6 --mem=8G --time=3:00:00 --job-name=EVAL \
