...
the companion `docs/archive/DYNAMICS_REVIEW.md`. The active training run referenced throughout is **Slurm job
...
> **Correction to `docs/archive/DYNAMICS_REVIEW.md`:** the `ShortcutForcing` **class docstring**
...
position-correlated coverage. `docs/archive/DYNAMICS_REVIEW.md` §4.1/§6.7 flags this as intentional-but-context-heavy.
...
MSE. `docs/archive/DYNAMICS_REVIEW.md` §6.1 rates this benign at small scale (K4 tracks K64 within ~1 dB).
...
(`:793-794`). The paper describes **only** the single appended token. `docs/archive/DYNAMICS_REVIEW.md` §6.3.
...
**Deviations noted in `docs/archive/DYNAMICS_REVIEW.md` §6.6:** RMSNorm eps hardcoded 1e-6 (not a config knob),
...
(`docs/archive/DYNAMICS_REVIEW.md` §1, §2.2). The dynamics input projection is `Linear(32→768)` per latent token
...
self-attn (`dynamics.py:137-321`); heads in `heads.py`. `docs/archive/DYNAMICS_REVIEW.md` §3.6, §6.5.

**Verdict: DEFERRED** (Phase 2+). The one-way agent-token masking rule is implemented consistently with
the paper. Reward/value twohot ranges deviate (±3 vs Dreamer's ±20; `docs/archive/DYNAMICS_REVIEW.md` §6.5) but are
...
Condensed from `docs/archive/DYNAMICS_REVIEW.md`; every claim `file:line`-cited there.
...
(`docs/archive/DYNAMICS_REVIEW.md` §2.1). Tokenizer v7 bottleneck **512×16**, folded to **(32,16,16)** dynamics
