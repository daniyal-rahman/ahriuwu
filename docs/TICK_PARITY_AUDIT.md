...
|---|---|---|---|
| 0 | `Map.Update(diff)` runs **before** `ObjectManager.Update(diff)` — `Game.cs:481,483`. Inside it: `CollisionHandler.Update()` → `PathingHandler.Update(diff)` → `MapScript.Update(diff)` → surrenders — `MapScriptHandler.cs:96-107` | wave spawning is `tick()` step 0, before movement — `step.py` "0. wave spawning" | **DIFFERS** (see gap 2: collision is inside this phase, and it runs before movement, not after) |
| 0a | Wave spawn timer, `LevelScript.Update` → `SetUpLaneMinion()` → `CreateLaneMinion(...)` → `ApiMapFunctionManager.CreateLaneMinion` → `ObjectManager.AddObject(m)` — `Map11/LevelScript.cs:152-188,263,283`; `APIMapFunctionManager.cs:135-144`. `_currentlyInUpdate` is false here (`ObjectManager.Update` hasn't started this frame), so `AddObject` takes the immediate branch and the minion lands directly in `_objects` — `ObjectManager.cs:223-236` | `step_waves_jax` + `spawn_minion`, step 0 — `step.py` | **AGREES**: a minion spawned this tick is already in the collection the per-object loop is about to run, so it moves/acts the same tick on both sides |
| 0b | `CollisionHandler.Update()` — `MapScriptHandler.cs:98`, `CollisionHandler.cs:121` — runs against positions as they stood at the **end of the previous tick** (no unit has moved yet this tick) | `resolve_collisions`, called **after** `step_move_units` in the same `tick()` call — `step.py` steps 1 then 1b | **DIFFERS** — see gap 2 |
| 1 | Per object, `ObjAIBase.Update` → `base.Update(diff)` (`AttackableUnit.Update`) runs first: `UpdateBuffs(diff)` — `AttackableUnit.cs:237,239` (buff bodies at `:805-827`, e.g. Garen's passive heal) | `step_buffs(...)` is called with **pre-tick** `state.x/state.y` even though it's written after the movement call — `step.py` "1a. buffs" | **AGREES** (data-dependency correct; see gap 5 for the misleading code layout) |
...
| 5 | `if (IsDead && _death != null) Die(_death)` — `AttackableUnit.cs:266-269` | death handled once, in the damage/kill section at the end of `tick()` | **DIFFERS** — see gap 3 (reward/removal can lag the HP-zero tick by one tick server-side; ours never lags) |
...
### Gap 1 — HP regen is not modeled at all (HIGH). **VERIFIED.**
...
- **Minions**: `BaseStaticHpRegen = 0` and no per-level regen ⇒
  `HealthRegeneration.Total = 0` ⇒ genuinely zero regen. Omitting regen for
  minions costs nothing.
- **Turrets**: `BaseStaticHpRegen = 3` ⇒ turrets sustain **3 HP/s** whenever
  below max HP. A turret that survives a poke and is left alone heals back up
  between skirmishes; our sim's turrets do not, so chip damage against a
  turret is strictly more effective in the sim than on the server.
- **Garen** has *two* separate healing mechanisms, on two different clocks,
  neither modeled:
  1. Ordinary `HealthRegeneration` (`BaseStaticHpRegen` + `HPRegenPerLevel =
     0.1`/level, `Garen.json:44` region) on the same 500 ms cadence as
     everyone else.
  ...
  worth taking, whether poke matters, and whether an all-in is lethal.

### Gap 2 — Collision is resolved before movement server-side, after movement in ours, and lags by a tick either way (HIGH). **VERIFIED.**
...
### Gap 3 — Kill rewards/removal can lag the HP-zero tick by one tick, keyed on iteration order (MEDIUM-HIGH). **VERIFIED.**
...
### Gap 4 — "Lowest-index-wins" is real, but the index is dictionary iteration order, not a stable per-unit ID (MEDIUM). **VERIFIED the mechanism, UNCERTAIN the exact reuse policy.**
...
### Gap 5 — Buff/movement code order is misleading but not a bug (LOW, informational). **VERIFIED.**
...
### Gap 6 — Dead units don't get an extra swing this tick, but do still run their AI/char scripts (LOW, informational). **VERIFIED.**
...
### Gap 7 — Missile-vs-movement ordering: already correct (not a gap). **VERIFIED.**
...
Yes. `AttackableUnit.Update` accumulates `diff` and calls `Stats.Update` once
per **500 ms of accumulated time**, not every 16.67 ms tick —
`AttackableUnit.cs:242-249`. `Stats.Update` adds
`HealthRegeneration.Total * diff * 0.001f` to `CurrentHealth`, clamped to max,
whenever `HealthRegeneration.Total > 0` and the unit isn't dead/full —
`Stats.cs:242-249`. `HealthRegeneration.BaseValue` comes straight from
`charData.BaseStaticHpRegen` (`Stats.cs:137`), matching the numbers already
in the sim's stat JSON: **0 for minions** (so minions get exactly zero regen
— nothing to model there) and **3 for turrets** (so turrets sustain 3 HP/s
whenever below max — this is real and unmodeled).

Garen regens on **two independent clocks**: the ordinary 500 ms
`HealthRegeneration` stat (base + `HPRegenPerLevel = 0.1`/level from
`Garen.json`), plus his unique passive `GarenPassiveHeal` buff, which runs its
own ~1000 ms internal timer and heals a level-scaled percentage of max HP
per second once 9 s out of combat, cancelled immediately on taking damage.
This is a *separate* mechanism from the stat-based regen (a direct
...
winnable and whether all-ins are lethal. Turret regen matters less but is not
zero either — it changes how much chip damage "sticks."
...
No new attack, but its scripts still run. See Gap 6 above: `CharScript.OnUpdate`
...
pre-tick position, matching this (see Gap 5). "Before or after the unit's own
...
- **Missile-vs-movement ordering** — already correctly modeled
  (advance-then-spawn); see Gap 7. No fix needed.
- **Turret's 0.7× damage multiplier vs minions** — not re-derived from the
  `Order3`/`Chaos3` basic-attack scripts/JSON (that's a damage-formula
  question, not a tick-ordering one, and out of this audit's brief). Spot
  checked `LaneTurret.AutoAttackHit` (`LaneTurret.cs`, ~lines 86-95) and found
  only the fountain-turret special case (flat 1000 true damage); the general
  0.7× rule lives in the basic-attack spell scripts, which `missiles.py`'s
  and `step.py`'s existing docstrings already cite accurately
  (`Spell.FinishCasting`, `IsMelee`/`HasEmptyScript` table) — took that as
  already audited rather than re-deriving.
...
- **Whether `_aiPaused` is ever set on death** — grepped every `PauseAI(`
  ...
  `Map8/LevelScriptObjects*.cs:106,109`); none is a death path. See Gap 6.
- **CoreCLR `Dictionary<TKey,TValue>` free-slot reuse semantics** —
  ...
  vendored in this repo, so Gap 4 / the iteration-order answer marks this
...
Highest-value fixes, in order: **(1)** model regen — turret static regen and
especially Garen's passive heal, since it's a distinct 1 s-cadence mechanism,
not free from adding generic `HealthRegeneration`; **(2)** swap the
...
VERIFIED ordering difference doesn't deserve; **(3)** be aware that gold/XP/
...
Missile timing and the dead-unit-can't-swing-again property are already
correct — don't spend time re-verifying either.
