...
| 2.7 | Dynamics | **WSD schedule with `decay_steps=0`** (warmup → flat forever; `utils/training.py:294` guards the decay branch with `if decay_steps > 0`, so it never fires). Paper states **no** LR schedule at all. **2026-09-02: severity raised LOW → HIGH (under test).** The dynamics ran 5,775 steps at a flat 3e-4 and never annealed. One fact consistent with four separate measurements in `DREAM_FIDELITY.md`: the plateau despite being under one epoch; blurry mean-seeking output (sharpness 26 vs 43 real); more denoising steps *raising* MSE; and a 1-step prediction (NMSE 0.0275) losing to copying the previous frame (0.0224). If it holds, a deviation filed as DELIBERATE/LOW is what blocks Phase 3. Test armed: `ops/anneal_probe.sh`. | DELIBERATE | **HIGH (under test)** |
