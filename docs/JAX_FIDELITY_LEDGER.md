...
a missing lever. **Gate 3 has exactly one cause**, `COLL-003`: engaged in lane
the two engines are identical to 0.1%, and the entire deficit comes from a
champion that cannot walk through a minion wave. **Gate 1** has two named,
...
| COLL-003 | `APPROX` | walking through a minion wave | The server's champion pushes through or around a wave; over a 600 s episode it spends **0** decisions walking with a minion inside 90 units. | The sim's champion **cannot get through a wave**. On the gate-3 respawn walk it travels 800 units of path for **1 unit of net displacement** over 811 decisions (t=462-489 s), wedged at a median nearest-minion distance of 66-70 units -- exactly champion radius 30 + minion radius 40. It spends **1,823** decisions of the episode walking with a minion inside 90 units. 1,337 of that walk's 2,543 decisions sit in the single lane bin 0.40-0.45. | **This is the whole of Gate 3.** It causes the extra death, which causes the 82% approach excess, which causes the five missing XP grants, which causes the level gap, which causes the +7.8% per-hit damage. Standing in lane the two engines' exposure is identical to 0.1%. | `sim/collision.py` against the server's `CollisionHandler`; `parity/gate3_swarm.py` reproduces the wedge. Suspect the escape/push direction resolution, not the trigger radii (`COLL-001` pins those). Compare the server's per-tick displacement for a champion in sustained contact with 3+ minions. |
