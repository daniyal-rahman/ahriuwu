...
class GarenAgent:
    ...
    def act_from_latent(self, latent, temperature=0.0, prev_action=None):
        ...
        # Sticky-categorical decode needs the previously EXECUTED movement bin.
        prev_idx = None
        if getattr(self.policy, "movement_gate", False) and self.act_buf:
            pm = torch.tensor(self.act_buf[-1]["movement"], device=self.device)
            prev_idx = self.policy.discretize_movement(pm).view(1, 1, 1, 2)
        with self._ac():
            ...
            abil, move, _ = self.policy.sample(h, temperature=temperature,
                                               prev_movement_idx=prev_idx)
