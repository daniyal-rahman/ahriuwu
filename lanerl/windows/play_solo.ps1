# Rendered RL inference, UNCONTESTED: the policy drives BLUE only.
#
# Red is left idle in its fountain. This is the case that produced the
# result worth seeing: the agent scores LOWER with no opponent (37.5 CS
# against 45.4 vs the bot), which no contested game explains.
#
...
function Log($m) { "$([DateTime]::Now.ToString('HH:mm:ss')) $m" | Out-File -Append C:\lanerl\play_solo.log -Encoding ASCII }
Remove-Item C:\lanerl\play_solo.log,C:\lanerl\server_solo.log,C:\lanerl\server_solo.err -ErrorAction SilentlyContinue
...
Start-Process -FilePath "C:\lanerl\winpub_rl\GameServerConsole.exe" `
  ...
  -RedirectStandardOutput C:\lanerl\server_solo.log -RedirectStandardError C:\lanerl\server_solo.err
Log "server started (control port 5200, bots off)"

foreach ($i in 1..90) { Start-Sleep -Seconds 1
  if ((Test-Path C:\lanerl\server_solo.log) -and (Select-String C:\lanerl\server_solo.log -Pattern "Game is ready" -Quiet)) { break } }
Log "server ready"
...
foreach ($i in 1..90) { Start-Sleep -Seconds 2
  if (Select-String C:\lanerl\server_solo.log -Pattern "Accepted client version" -Quiet) { break } }
Log "client connected"

foreach ($i in 1..300) { Start-Sleep -Seconds 1
  if (Select-String C:\lanerl\server_solo.log -Pattern "LANERL_CONTROL client attached" -Quiet) { break } }
if (Select-String C:\lanerl\server_solo.log -Pattern "LANERL_CONTROL client attached" -Quiet) {
