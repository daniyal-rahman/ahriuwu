...
sleep 60
echo "not-in-manifest errors: $(grep -c 'not in the manifest' "$OUT/rads_F.log" 2>/dev/null)"
echo "waiting for game start ..."
for i in $(seq 1 60); do grep -q "Patience is over" "$OUT/server_f.log" 2>/dev/null && { echo "GAME STARTED"; break; }; sleep 3; done
sleep 20
for t in 1 2 3; do
