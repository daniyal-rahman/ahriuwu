...
for p in "lol_game_client/releases/0.0.1.7" "lol_game_client_en_gb/releases/0.0.0.235" \
         "LEVELS/releases/0.0.0.235" "DATA/releases/0.0.0.235"; do
    mkdir -p "$PROJ/$p"
...
cp -f "$PROJ/lol_game_client_en_gb/releases/0.0.0.235/releasemanifest" "$PROJ/LEVELS/releases/0.0.0.235/releasemanifest"
cp -f "$PROJ/lol_game_client_en_gb/releases/0.0.0.235/releasemanifest" "$PROJ/DATA/releases/0.0.0.235/releasemanifest"
...
"$DOTNET_ROOT/dotnet" ./GameServerConsole.dll --config "$NFS/ahriuwu-lanerl/lanerl/cfg/garen1v1.json" \
    >"$OUT/server_e.log" 2>&1 & SRVPID=$!
for i in $(seq 1 40); do grep -q "Game is ready" "$OUT/server_e.log" 2>/dev/null && break; sleep 2; done
echo "server ready"

echo "=== [3/4] client ==="
cd "$CLIENT"
timeout 320 "$WINE" "League of Legends.exe" "8394" "LoLLauncher.exe" "" \
    "127.0.0.1 5119 17BLOhi6KZsTtldTsizvHg== 1" >"$OUT/rads_E.log" 2>&1 &
sleep 60
echo "not-in-manifest errors: $(grep -c 'not in the manifest' "$OUT/rads_E.log" 2>/dev/null)"
echo "waiting for game start ..."
for i in $(seq 1 60); do grep -q "Patience is over" "$OUT/server_e.log" 2>/dev/null && { echo "GAME STARTED"; break; }; sleep 3; done
sleep 20
for t in 1 2 3; do
    ffmpeg -loglevel error -y -f x11grab -video_size 1920x1080 -i :99 -frames:v 1 "$OUT/shots/E_$t.png" >/dev/null 2>&1
    sleep 20
done

echo "=== [4/4] verdict ==="
echo "not-in-manifest: $(grep -c 'not in the manifest' "$OUT/rads_E.log")"
echo "loaded level?  $(grep -icE 'level|loadscreen|hud|champion' "$OUT/rads_E.log")"
grep -iE "level|spawn|hud|champion|connected" "$OUT/rads_E.log" 2>/dev/null | tail -8
echo "--- server:"; grep -vE "WARN|Could not find" "$OUT/server_e.log" | tail -6
