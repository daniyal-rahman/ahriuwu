...
export WINEPREFIX="$ROOT/prefix" DISPLAY=:1   # REAL Xorg on the 5080, not Xvfb
...
# No Xvfb: a real Xorg (:1) is running on the GPU. Xvfb was the blocker -- it has
# no DRI3/Present, so NVIDIA Vulkan segfaulted in vkGetPhysicalDeviceSurfaceCapabilitiesKHR
# even after /dev/dri access was granted, proving the surface (not permissions) was the issue.
XP=""
...
for t in $(seq 1 $((DUR/60))); do
    sleep 60
    ffmpeg -loglevel error -y -f x11grab -video_size 1920x1080 -i :1 -frames:v 1 "$OUT/shots/gpu_$t.png" >/dev/null 2>&1
