#!/bin/bash
# ACTION-CONDITIONED MIXED dynamics run on Vast (the paper recipe): provision 4x4090,
# warm-start from the action-conditioned seed (135 lineage, on R2), train replays
# (real actions) + YT (no_action_embed) with PLAIN LATENT loss (the Step-1' fix, NOT
# the pixel-HUD loss), micro-epoch eval (H=8 rollout gate every 150 steps), stream
# ckpt->R2 every cycle, auto-stop on PLATEAU or when the $BUDGET_USD cap hits, then
# destroy. Streams keep R2 current so worst-case nothing is lost/overspent.
set -uo pipefail
export PATH=/home/dani/.vastcli/bin:/home/dani/bin:$PATH
VAST=/home/dani/.vastcli/bin/vastai
KEY=/home/dani/.ssh/id_ed25519
LOG=/srv/nfs/projects/ahriuwu/scratchpad/vast_action.log
: > "$LOG"
say(){ echo "[$(date '+%T')] $*" | tee -a "$LOG"; }
BUDGET_USD="${BUDGET_USD:-20}"
R2=r2:ahriuwu-yt-pretrain
SEED="${SEED_R2:-$R2/dynamics_action_accel/dynamics_latest.pt}"   # resume the mixed run; falls back set by caller
CKR2="$R2/dynamics_action_accel"
...
shipped=0
for a in 1 2 3 4; do
  if tar czf - -C /srv/nfs/projects/ahriuwu src scripts | SSH 'mkdir -p /root/ahriuwu && tar xzf - -C /root/ahriuwu' && SSH 'test -f /root/ahriuwu/scripts/train_dynamics.py'; then shipped=1; break; fi
  say "ship attempt $a failed (transient SSH?); retry in 12s..."; sleep 12
done
[ "$shipped" = 1 ] || { say "ship failed after 4 tries"; exit 1; }
for a in 1 2 3; do
  SSH 'mkdir -p /root/.config/rclone && cat > /root/.config/rclone/rclone.conf' < /home/dani/.config/rclone/rclone.conf && break
  say "rclone-config push attempt $a failed; retry"; sleep 8
done
...
mkdir -p /workspace/seed /workspace/ckpt
cd /root/ahriuwu
echo "staging mixed action-conditioned latents + labels + seed..."; DEST=/workspace SRC=/root/ahriuwu R2_SEED=$SEED bash scripts/stage_dyn_action.sh
echo "seed \$(stat -c%s /workspace/seed/seed.pt 2>/dev/null) bytes"
export NGPU=$NGPU NCCL_SHM_DISABLE=0
export DYN_ARGS_FILE=scripts/dyn_train_args_action.sh
export LATENTS_DIR=/workspace/latents_action CHECKPOINT_DIR=/workspace/ckpt LABELS_ROOT=/workspace/labels_action
...
BEST=0; NOIMP=0; NEVAL=0; LAST_EVAL=0
for i in $(seq 1 "$ITERS"); do
  ...
  # plateau check — gate on the ROLLOUT (real dream quality), counting only
  # DISTINCT evals (dedupe by eval step; the monitor polls faster than evals
  # occur, so a naive tail re-reads the same eval and false-plateaus).
  EV=$(SSH 'grep -aoE "EVAL-ROLLOUT step [0-9]+" /workspace/train.log 2>/dev/null | tail -1' 2>/dev/null | grep -oE "[0-9]+$")
  if [ -n "$EV" ] && [ "${EV:-0}" -gt "${LAST_EVAL:-0}" ]; then
    LAST_EVAL=$EV
    RMEAN=$(SSH 'grep -aoE "EVAL-ROLLOUT.*psnr_mean=[0-9.]+" /workspace/train.log 2>/dev/null | tail -1' 2>/dev/null | grep -oE "psnr_mean=[0-9.]+" | grep -oE "[0-9.]+")
    TAU=$(SSH 'grep -aoE "psnr_tau0.9=[0-9.]+" /workspace/train.log 2>/dev/null | tail -1' 2>/dev/null | grep -oE "[0-9.]+" | tail -1)
    IMP=$(python3 -c "print(1 if ${RMEAN:-0} > $BEST + 0.15 else 0)" 2>/dev/null || echo 0)
    if [ "$IMP" = "1" ]; then BEST=$RMEAN; NOIMP=0; else NOIMP=$((NOIMP+1)); fi
    NEVAL=$((NEVAL+1))
    say "    eval#$NEVAL step=$EV rollout_mean=${RMEAN:-?} tau0.9=${TAU:-?} best=$BEST noimp=$NOIMP/12"
    # require a real training budget past the warm-start before allowing plateau-stop
    if [ "$NOIMP" -ge 12 ] && [ "$NEVAL" -ge 20 ]; then
      say "PLATEAU (rollout_mean no new best in 12 distinct evals, $NEVAL evals seen) -> stop early"; break
    fi
  fi
done
