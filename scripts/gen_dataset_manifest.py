...
def write_index(mans):
    """Top-level docs/archive/DATASETS.md — one table over all datasets. Regenerated on every --all run."""
    ...
    open(os.path.join(REPO_DOCS, "archive", "DATASETS.md"), "w").write("\n".join(L) + "\n")
    print(f"  ✓ index -> docs/archive/DATASETS.md ({len(mans)} datasets)")
