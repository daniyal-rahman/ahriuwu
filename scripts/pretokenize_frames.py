...
Converts frames to latent tensors using trained transformer tokenizer.

Usage:
    python scripts/pretokenize_frames.py --checkpoint checkpoints/transformer_tokenizer_best.pt

    # Resume interrupted encoding
    python scripts/pretokenize_frames.py --checkpoint checkpoints/transformer_tokenizer_best.pt --resume

    # Manual batch size override
    python scripts/pretokenize_frames.py --checkpoint checkpoints/transformer_tokenizer_best.pt --batch-size 16

Output:
    {output_dir}/{video_id}/latent_{frame:06d}.npy
    Shape: (latent_dim, 16, 16) float16 numpy array
"""
...
def estimate_batch_size(gpu_memory_gb: float) -> int:
    """Estimate safe batch size based on GPU memory.

    Memory estimate: ~400MB per batch item (attention + embeddings).
...
def main():
    ...
    # Load tokenizer
    print("\nLoading tokenizer...")
    model, latent_dim = load_tokenizer(checkpoint_path, args.device)
    print(f"Loaded transformer tokenizer with {model.get_num_params():,} parameters")
