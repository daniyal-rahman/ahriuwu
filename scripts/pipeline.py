...
# Shifts from 16.7: RVAs +0x18240, spellbook/champion_name region +0x38,
# level/gold_total/vision region +0x40, stats_base +0x38 (sub-offsets unchanged).
...
OFFSETS = {
    ...
    # hero struct — inline fields (patch 16.8, API-verified 2026-04-16)
    "position":       0x25C,     # vec3 f32 (GameObject base, unchanged)
    "hp_current":     0x1080,    # f32 (unchanged)
    "hp_max":         0x10A8,    # f32 (unchanged)
    "gold_current":   0x2868,    # f32 — +0x38 from 16.7's 0x2830
    "gold_earned":    0x2890,    # f32 — +0x38 from 16.7's 0x2858 (current held gold)
    "active_spell":   0x3158,    # ptr (SpellBook+0x38)
    "champion_name":  0x4360,    # ASCII str (SSO inline)
    "level":          0x4D50,    # u32 (verified: 4→9 API-matched)
    "kills":          0x5468,    # u32 — NOT in hero struct per prior note; keep for back-compat
    "assists":        0x54C8,    # u32 — same caveat; 0x5540 also plausible (A=0→1)
    "vision_score":   0x5618,    # f32 — verified against API wardScore
    "click_dest_rva": 0x1DC2FA0, # UNVERIFIED for 16.8 — AiWaypoints hunt deferred
    "click_dest_off": 0x20,

    # SpellBook (inline at hero+0x3120) — verified via GarenQ/W/E/R deref chain
    "spellbook":      0x3120,
    "slot_array":     0xAE0,     # relative to spellbook — array of 6+ slot ptrs, stride 8
    # per-slot fields (unchanged — verified by spell-name deref hit):
    "slot_level":     0x28,      # u32
    "slot_cd_expire": 0x30,      # f32 — ABSOLUTE game time when CD ends
    "slot_total_cd":  0x74,      # f32 — base cooldown for current rank
    "slot_spell_info": 0x130,    # ptr

    # SpellInfo struct (reached via slot_spell_info) — unchanged
    "spell_name_ptr": 0x28,

    # ActiveSpellCast chain — unchanged
    "spell_info":     0x008,
    "cast_target":    0x0DC,

    # HeroStats sub-region — stats_base shifted +0x38, sub-offsets unchanged
    "stats_base":     0x1BC0,
    "stat_ad":        0x348,     # verified: 0x1BC0+0x348 = 0x1F08 = 99.33 at L9
    "stat_armor":     0x4D8,     # verified: 0x1BC0+0x4D8 = 0x2098 = 83.31
    "stat_mr":        0x528,     # verified: 0x1BC0+0x528 = 0x20E8 = 59.45
    "stat_move_speed":0x5C8,     # verified: 0x1BC0+0x5C8 = 0x2188 = 379.59
    "stat_atk_range": 0x618,
}
...
def classify_spell(name):
    """Classify a Garen spell name into action type.
...
def _mem_loop(m, base, hero_ptrs, gt_rva, stop, results):
    ...
    while not stop.is_set():
        ...
        for name, hinfo in hero_ptrs.items():
            ...
            # Only works for Garen (the replay-followed hero) since it's a single cached ptr
            wp = None
            if name == "Garen":
                aim_ptr = m.u64(base + o["click_dest_rva"])
                if aim_ptr and 0x10000 < aim_ptr < 0x7FFFFFFFFFFF:
                    wp = m.vec3(aim_ptr + o["click_dest_off"])
            entry = {
