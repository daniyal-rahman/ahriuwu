...
try:
    ...
    evals.append({'update': 760, 'summary': {'0': {'mean_cs': 19.4}, '1': {'mean_cs': 16.0}}})
    _, _, per0 = load(roots[0])
    ex = [v['update'] * per0 / 1e6 for v in evals]
    ax.scatter(ex, [v['summary']['0']['mean_cs'] for v in evals], marker='D', s=60, color='#1a1a19', zorder=5, label='frozen eval, blue mean (5 ep)')
    ax.scatter(ex, [v['summary']['1']['mean_cs'] for v in evals], marker='s', s=60, facecolors='none', edgecolors='#1a1a19', zorder=5, label='frozen eval, red mean (5 ep)')
...
ax.set_title('Mirror self-play on the C# server, 2026-09-25 (near-wave start, 10 Hz)')
