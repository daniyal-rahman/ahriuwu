...
#
# One process for the whole suite OOM-kills at the 14G cap around test 58:
...
for f in lanerl_jax/sim/tests/test_*.py; do
    if out=$(ops/login_capped.sh 14G 4 .venv-jax/bin/python -m pytest -q -p no:cacheprovider "$f" "$@" 2>&1); then
