"""LEGACY: Pre-unification tokenizer for loading old checkpoints.

THIS FILE IS A VESTIGE OF THE OLD ATTENTION IMPLEMENTATION (pre-commit a09d2e0).
It exists solely to load tokenizer checkpoints trained before the attention
unification refactor. The unified Attention class changed internal computation
in a way that's incompatible with these weights.

DO NOT use this for new training. Use transformer_tokenizer.py instead.
Remove this file once all old checkpoints have been retrained.
